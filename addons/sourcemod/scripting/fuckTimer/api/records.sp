...
public void GetPlayerRecords(HTTPResponse response, any userid, const char[] error)
{
    if (response.Status != HTTPStatus_OK)
    {
        SetFailState("[Records.GetPlayerRecords] Something went wrong. Status Code: %d, Error: %s", response.Status, error);
        return;
    }
    ...
    if (jMainrecords.Length < 1)
    {
        LogMessage("[Records.GetPlayerRecords] We didn't found any records for this map...");
        return;
    }
    ...
    LogMessage("[Records.GetPlayerRecords] We found %d records for this map", jMainrecords.Length);
